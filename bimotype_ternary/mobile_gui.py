...
def main(page: ft.Page):
    ...
    state["local_fp"] = state["peer"].start_listening(
        thread_callback=peer_thread_callback
    )
        ...
        for req_fp in state["handshake_requests"]:
            ...
            def acc_click(e, fp=req_fp, t_data=target_data):
                ...
                page.snack_bar = ft.SnackBar(
                    ft.Text(f"Conexión establecida con {fp[:8]}"),
    ...
    def send_click(e):
        ...
        if not target_fp:
            page.snack_bar = ft.SnackBar(
                ft.Text("Introduce un Fingerprint de destino"),
                bgcolor=ft.Colors.RED_700
            )
            page.snack_bar.open = True
        elif target_fp not in state["peer"].trusted_peers:
            page.snack_bar = ft.SnackBar(
                ft.Text("Aún no es de confianza. Solicita Handshake primero."),
                bgcolor=ft.Colors.RED_700
            )
            page.snack_bar.open = True
        elif prompt:
            ...
            if target_data:
                state["peer"].send_packet(
                    target_data[0], target_data[1], prompt, target_fp
                )
                msg_input.value = ""
            update_chat_ui()
        else:
            page.snack_bar = ft.SnackBar(
                ft.Text("Huella de destino no encontrada"),
                bgcolor=ft.Colors.RED_700
            )
            page.snack_bar.open = True
        page.update()
    ...
    def pick_files_result(e: ft.FilePickerResultEvent):
        ...
        try:
            ...
            protocol = QRTransferProtocol(
                h7_index=state["h7_seed"], chunk_size=400
            )
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text(f"Archivo dividido en {len(frames)} QRs."),
                bgcolor=ft.Colors.GREEN_700
            )
            page.snack_bar.open = True
    ...
    def scan_qr_click(e):
        page.snack_bar = ft.SnackBar(
            ft.Text("Abriendo escáner... (Asegúrate de conceder permisos)"),
                ...
                if file_bytes and filename:
                    ...
                    try:
                        ...
                        with open(out_path, "wb") as f:
                            f.write(file_bytes)
                        page.snack_bar = ft.SnackBar(
                            ft.Text(f"¡Éxito! Guardado en {out_path}"),
                            bgcolor=ft.Colors.GREEN_700
                        )
                    except Exception:
                        with open(filename, "wb") as f:
                            f.write(file_bytes)
                        page.snack_bar = ft.SnackBar(
                            ft.Text(f"¡Éxito! Guardado como {filename}"),
                            bgcolor=ft.Colors.GREEN_700
                        )
                else:
                    page.snack_bar = ft.SnackBar(
                        ft.Text("Escaneo cancelado o fallido."),
                        bgcolor=ft.Colors.RED_700
                    )
                page.snack_bar.open = True
                page.update()
